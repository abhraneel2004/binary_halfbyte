...
const MemoizedProfileSetup = memo(ProfileSetup);
...
function App() {
  ...
  const completeProfileSetup = useCallback(() => setShowProfileSetup(false), []);
  ...
  return (
    <Router>
      <div className="min-h-screen flex flex-col bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white transition-colors duration-200">
        ...
        <ConditionalFooter />
        <MemoizedSettingsMenu isOpen={showSettings} onClose={closeSettings} />
        {showProfileSetup && <MemoizedProfileSetup onComplete={completeProfileSetup} />}
      </div>
    </Router>
  );
}
...
const Header = memo(({ user, darkMode, toggleDarkMode, openSettings }) => (
  <header className="sticky top-0 bg-white dark:bg-gray-800 shadow-md z-50">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
      <div className="flex justify-between items-center">
        <Link to="/" className="flex items-center space-x-2">
...
// Extracted ConditionalFooter component
const ConditionalFooter = memo(() => {
  const shouldShowFooter = window.location.pathname === '/profile';
  return shouldShowFooter ? <MemoizedFooter /> : null;
});
